...
def build_transport_option_swap_payload(existing_option_payload: Dict[str, Any],
    ...
    CONFIRMED REAL BUG (product owner, 2026-09-16, live "AC First Class Seat" bracket): this used
    to always rewrite the name via _swap_route_text, which - same as the parent's name/datasheet
    name - falls back to appending "(return)" when the route isn't literally in the text. That
    fallback makes sense for a TRANSPORT's own name (which genuinely does describe a route), but
    an option's name describes a SERVICE CLASS and pax range, not a direction - real option names
    generated by this app deliberately never mention the route at all (see
    build_transport_payloads' own option_name assembly: "The route is already the product's own
    name, so repeating it here made every modality read 'Luxor - Hurghada - 1 Pax'"), so almost
    every real option name hits that fallback and gets a nonsensical "AC First Class Seat
    (return)". Uses _swap_route_text_if_found instead: only touches the name when it genuinely
    does spell out the route, leaves it exactly as-is otherwise (the overwhelmingly common case)."""
    ...
    if en.get("name"):
        ...
        en["name"], _swapped = _swap_route_text_if_found(en["name"], new_arrival_name, new_departure_name)
        translations["EN"] = en
