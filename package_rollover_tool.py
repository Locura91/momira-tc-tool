"""
package_rollover_tool.py â€” "Package Rollover" prototype screen: a human enters ONE Holiday
...
confirmed rules (see package_rollover_rules.py), and shows it for a human to review â€” see the
...
one-Package-ID-at-a-time tool rather than an automatic scan of every package on the site â€”
this screen is exactly that. It makes REAL, LIVE GET calls to Travel Compositor (the
Packages API is already reachable with the existing credentials â€” see
travelcompositor_api.py's get_holiday_package_info / get_holiday_packages, already used by the
Holiday Package translation sync). It does NOT call PUT / update anything â€” the exact
...
the human see the new departure/price" â€” the best case Chris described is the human applying
...
anywhere in this file or in package_rollover_rules.py â€” enforced by
...
def _get_package_api_client():
    ...
    .client) â€” the Packages GET/PUT methods only exist on travelcompositor_api.py's client
...
def _render_field_note(label, value, field_name, unit=""):
    if field_name is None:
        st.markdown(f"**{label}:** *(not found in the response â€” see raw data below)*")
    else:
...
def _current_price_from(info, day_to_day):
    """Tries the info response first, then day-to-day â€” whichever has a recognizable price
    ...
    "pricePerPerson"), not a guessed/hardcoded label â€” see _render_field_note's docstring
...
def _current_departure_from(info, day_to_day):
    """Same idea as _current_price_from, for the CURRENT departure date â€” needed for the
...
def render_package_rollover_tool():
    st.subheader("ðŸ” Package Rollover (prototype)")
    st.warning("âš ï¸ **Prototype â€” read-only, nothing is ever booked or written from this "
              "screen.** It looks up a real package and proposes a replacement departure "
              "using the confirmed rules (under 14 days out â†’ roll to ~4 months ahead, hotel "
              "rating 8+, price within +3.5% of today's live price). It makes real GET calls "
              "to Travel Compositor but has no PUT/write call anywhere in its code â€” that's "
              ...
              "package-edit screen** â€” exactly the same click-Save you already do today. See "
              "the \"package-auto-rollover-rules\" project note for the full picture.")
    st.info("ðŸ“Œ **Scope, confirmed 2026-08-21:** a Holiday Package ID that's ONLY a ClosedTour "
           "(e.g. a fixed-schedule cruise) doesn't need this tool â€” its departures are managed "
           "through the normal ClosedTour update flow instead. Use this tool for a package "
           "that's fully dynamic (no ClosedTour component â€” any day works) or one built around "
           "a ClosedTour PLUS dynamic flights and a pre/post program. **For now, testing is "
           "focused on fully dynamic packages (no ClosedTour component)** â€” those are the ones "
    ...
    if st.button("ðŸ” Look up package", type="primary", disabled=not package_id.strip()):
        try:
    ...
    for label, payload in (("Package info", info),
        ...
        if isinstance(payload, dict) and "error" in payload:
            st.error(f"{label}: Travel Compositor returned an error â€” "
    ...
    if current_departure is not None:
        ...
        if prr.is_departure_closed(current_departure, today):
            st.error(f"ðŸ”´ This departure is {days_out} day(s) away â€” under the confirmed "
                    f"14-day trigger, this package NEEDS rolling.")
        else:
            st.success(f"âœ… This departure is {days_out} day(s) away â€” still well outside the "
                      f"confirmed 14-day trigger, so it does NOT need rolling yet. The "
                      f"proposal below is shown for testing/preview only.")
    else:
        st.warning("âš ï¸ Couldn't determine the current departure date, so it's unknown whether "
                  "this package actually needs rolling yet.")

    if isinstance(calendar, dict) and "error" in calendar:
        st.info("Can't propose a replacement departure â€” the calendar call failed (see error "
               "above).")
    else:
        ...
        if not candidates:
            ...
            st.markdown("###### Old â†’ New (what you'd change in Travel Compositor)")
            dcol1, dcol2 = st.columns(2)
            with dcol1:
                st.markdown("**Departure date**")
                old_dep = current_departure.isoformat() if current_departure else "*(unknown)*"
                st.markdown(f"{old_dep} â†’ **{dyn['proposed_date'].isoformat()}**")
            with dcol2:
                st.markdown("**Price**")
                st.markdown(f"{current_price if current_price is not None else '*(unknown)*'} â†’ "
            ...
            st.info("No calendar of fixed departure dates was found for this package, which is "
                   "the normal case for a dynamic package â€” any day can work, so this proposes "
                   "today + ~4 months directly rather than matching against a calendar entry.")
            if dyn["shifted_for_blackout"]:
                st.warning(f"âš ï¸ The plain ~4-months-out date ({dyn['target_date'].isoformat()}) "
                          f"fell inside a no-departure window ({dyn['blackout_reason']}), so "
                          f"the proposal was pushed forward to the next clear date.")
            st.warning("âš ï¸ **This doesn't check price or hotel rating automatically** â€” a "
            ...
            st.warning("âš ï¸ **If this package includes a fixed-departure component** (e.g. a "
                      "cruise or coach tour that only departs on specific weekdays, like a "
                      "Nile cruise), this proposed date is NOT checked against that schedule â€” "
                      "confirm it matches before applying it in Travel Compositor.")
            st.info("ðŸ‘‰ **This app never applies this change.** Open this package in Travel "
                   "Compositor's own edit screen, enter this departure date the same way you "
                   "would for any regular update, and click **Save there** â€” the normal, "
            ...
            if result["status"] == "no_dated_candidates":
                st.info(f"Found {result['candidates_seen']} calendar entr(y/ies) but couldn't "
                       f"parse a future date from any of them â€” see raw data below.")
            elif result["status"] == "no_qualifying_candidates":
                if result.get("below_minimum_test_coverage"):
                    st.warning(f"âš ï¸ Only {result['candidates_tested']} future departure day(s) "
                              f"were available to test â€” fewer than the minimum "
                ...
                with ocol1:
                    st.markdown("**Departure date**")
                    old_dep = current_departure.isoformat() if current_departure else "*(unknown)*"
                    st.markdown(f"{old_dep} â†’ **{best['date'].isoformat()}**")
                with ocol2:
                    st.markdown("**Price**")
                    ...
                    st.markdown(f"{old_price} â†’ **{new_price}**")
                if result.get("picked_by") == "cheapest_price":
                    st.caption(f"âœ… Picked as the **cheapest** of {result['candidates_tested']} "
                ...
                with st.expander(f"ðŸ“‹ All {result['candidates_tested']} departure day(s) tested"):
                    rows = []
                    for c in result.get("qualifying") or []:
                        rows.append({"date": c["date"].isoformat() if c["date"] else None,
                                    "price": c["price"], "rating": c["rating"],
                                    "status": "ðŸ‘‰ recommended" if c is best else "qualifying"})
                ...
                if result["rating_unverifiable"]:
                    st.warning("âš ï¸ Couldn't find a rating field on this candidate â€” the 8+ "
                ...
                if result["price_unverifiable"] or current_price is None:
                    st.warning("âš ï¸ Couldn't find a price field to compare â€” the +3.5% cap "
                              "could NOT be checked automatically. Check the price in Travel "
                              "Compositor yourself before applying this.")

                st.info("ðŸ‘‰ **This app never applies this change.** Open this package in "
                       ...
                       "the hotel's rating, and click **Save there** â€” the normal, manual "
                       "step you already do today.")

    with st.expander("ðŸ”Ž Raw API responses"):
