...
folds what_to_bring into voucher_remarks once, up front, using the exact same single-line format
(builder.format_what_to_bring_line) the builder itself used to append at publish time - and
clears what_to_bring so the builder's still-shared append logic (used by the other four
products, which keep two fields) finds nothing left to add and doesn't double it up.

UPDATED (product owner, 2026-09-03): "if mentioned What to bring and we include them in the
Voucher remark, just write it like this: 'What to bring: Example 1, Example 2 etc'" - one
comma-separated line, not the previous "What to bring:\n{items}" block.
"""
...
def test_merges_what_to_bring_into_voucher_remarks_with_a_blank_line():
    ...
    assert data["voucher_remarks"] == (
        "Please arrive 15 minutes early.\n\nWhat to bring: Passport, sun cream, hat")
...
def test_merge_with_no_existing_voucher_remarks_uses_the_block_alone():
    ...
    merge_what_to_bring_into_voucher_remarks(data)
    assert data["voucher_remarks"] == "What to bring: Passport, sun cream"
    assert data["what_to_bring"] == ""
