...
def test_build_proposals_flags_unchanged_when_current_matches_new():
    ...
    # REVERTED 2026-09-11: the cancellation text lives in description's HTML paragraphs, same
    # as it always did (see this module's own top-of-file docstring) - a record whose
    # description paragraph already matches the synthesized new text is the unchanged case.
...
def test_apply_updates_cancellation_ranges_and_description():
    client = _FakeTransportClient(transports=[_sample_record()])
    rows, _ = cbt.load_supplier_transports_for_cancellation(client, "SUP-X")
    proposals = cbt.build_proposals(rows, [{"days": 14, "fee_percentage": 50.0}])
    ...
    # REVERTED 2026-09-11 (real production evidence, see this module's own top-of-file
    # docstring): the new cancellation sentence lands in description, same as it always did -
    # Transport has no working voucherRemarks field via Travel Compositor's real API.
