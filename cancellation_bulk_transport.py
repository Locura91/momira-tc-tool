...
WHERE THE CANCELLATION TEXT LIVES ON A LIVE TRANSPORT: inline inside `description`'s HTML
"<p>...</p>" paragraphs - see _swap_cancellation_paragraph() for how the right paragraph is
found without disturbing the service description, an optional "What to bring:" block, or a
manual note. Changing the STRUCTURED cancellationRanges field alone would leave the customer-
facing text describing the OLD policy, so this module rewrites both together.

REVERTED 2026-09-11 (same day, product owner, real production evidence): this module briefly
wrote the cancellation sentence into Voucher remarks instead, on the belief that Transport had a
genuine, separately-persisted voucherRemarks field (schemas.py's TransportDataSheetVO.
voucherRemarks, added 2026-09-10 based only on a screenshot of Travel Compositor's own Transport
edit page showing a "Voucher remarks" input box). That belief was WRONG: the product owner
pasted Travel Compositor's actual Swagger for `PUT /transport/{supplierId}` (the real endpoint
this module's own update_transport calls hit) and its `datasheets.<lang>` type
(ContractTransportDataSheetVO) has ONLY `name` and `description` - no voucherRemarks field
exists anywhere in Transport's real API schema, on either the parent record or an option. Every
write this module sent to voucherRemarks was silently dropped by Travel Compositor - confirmed
by a live debug capture (the PUT response echoed back a datasheet with no voucherRemarks key at
all) and a live screenshot (the field on Travel Compositor's own page stayed empty). Worse, one
run of this reasoning also shipped a one-off repair tool (bulk_notes._plan_transport_
cancellation_text_repair) that REMOVED the cancellation paragraph from live Description on the
belief the replacement was landing in Voucher remarks - it wasn't, so several MOMIRA_TEST/
MOMIRA_EG_FT Transports briefly had no cancellation text in either field until this module was
reverted and re-run against them. See claude/transport-bulk-write-silent-drop-full-in-list-
2026-09-11.md and the follow-up doc in the project for the full incident.
...
def _current_cancellation_snippet(description_html: str) -> Optional[str]:
    """The plain-text content of the FIRST "<p>...</p>" paragraph that mentions cancellation, or
    None if none does. See _swap_cancellation_paragraph for why "first match" is the safe
    choice, not just "any match"."""
...
def build_proposals(rows: List[Dict[str, Any]], new_tiers) -> List[Dict[str, Any]]:
    ...
    proposals = []
    for row in rows:
        new_description_html, existing_found = _swap_cancellation_paragraph(
            row["description_html"], new_text)
        unchanged = (
...
def apply_proposals(client, supplier_id: str, proposals: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    REVERTED 2026-09-11 (see this module's own top-of-file docstring): the cancellation
    sentence goes into description's HTML paragraphs, same as it always did - Transport has no
    working voucherRemarks field via Travel Compositor's real API."""
    results = []
    for p in proposals:
        ...
        for lang, sheet in datasheets.items():
            sheet = dict(sheet or {})
            new_description, _found = _swap_cancellation_paragraph(sheet.get("description") or "", new_text)
